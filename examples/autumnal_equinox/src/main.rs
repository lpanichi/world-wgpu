/// Autumnal Equinox validation example.
///
/// Validates that on the September equinox (~day 265):
/// - Solar declination is ≈ +0.00° (maximum)
/// - The subsolar point is at latitude ≈ +0.00° (Equator)
/// - The Sun direction has a significant zero Z component in ECI
/// - Earth-Sun line is aligned from the equatorial plane
...
impl EquinoxSimulation {
    fn new() -> Self {
        // September 22, 2025 12:00 UTC — approximate autumnal equinox
        let equinox_time = Utc.with_ymd_and_hms(2025, 9, 22, 12, 0, 0).unwrap();
        let (day, hour) = Astral::datetime_to_day_hour(&equinox_time);
        ...
        // faces the viewer. Nudged slightly off-axis so the Sun-direction
        // line's label doesn't sit right on the camera's line of sight.
        let sun_dir_vec =
        ...
        let mut core_sim = System::builder().build(equinox_time);
        ...
        // Equator line (≈0.00°N) — mark several points along it
        for lon in (-180..=180).step_by(30) {
            core_sim.shapes.add_surface_point(0.00, lon as f32, "");
        }
        ...
        let validation_info = format!(
            "AUTUMNAL EQUINOX VALIDATION — Day {} ({}) | \
             Declination: {:.4}° (expect ≈+0.00°) | \
             Subsolar: ({:.2}°, {:.2}°) | \
             Sun ECI Z: {:.4} (positive = north tilt)",
            day,
            equinox_time.format("%Y-%m-%d %H:%M UTC"),
            declination,
            subsolar_lat,
            subsolar_lon,
            sun_dir[2],
...
impl iced::widget::shader::Program<Message> for EquinoxSimulation {
...
fn update(sim: &mut EquinoxSimulation, message: Message) -> iced::Task<Message> {
    match message {
        Message::Tick => {
...
fn view(sim: &EquinoxSimulation) -> Element<'_, Message> {
...
fn main() -> iced::Result {
    ...
    let mut app = iced::application(EquinoxSimulation::new, update, view);
    if let Some(size) = gui::screenshot::window_size() {
        app = app.window_size(size);
    }
    app.subscription(|_state: &EquinoxSimulation| {
